  ...
  components: { layoutKaryaMamusVideo, karyaMamusVideo },
        ...
        volume(){
            ...
            ChangeId(id){
                this.id = id;
                if(id ==1){
                    this.title = 'True Colors'
                }
                this.getKarya()
            },
            async getKarya(){
                const allKarya = this.$fire.firestore.collection('mamus-video').doc(this.title)
                try{
                    const video = await allKarya.collection('Karya').doc('Video').get() // fotonya
                    ...
                    this.vid = Object.values(video.data())
